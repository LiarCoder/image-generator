...
/**
 * Pixel dimension estimation from target byte size and format.
 */
export class ImageSizer {
   ...
   */
  static #dimensionsForPixelBudget(totalPixels) {
   ...
   * @returns {{ width: number, height: number }}
   */
  static #clampToMinDimensions(width, height) {
    return {
   ...
   * @returns {{ width: number, height: number }}
   */
  static calculate(targetBytes, format) {
    const bpp = BYTES_PER_PIXEL[format] ?? UNKNOWN_FORMAT_BPP;
    const totalPixels = targetBytes / bpp;
    const { width, height } = ImageSizer.#dimensionsForPixelBudget(totalPixels);
    return ImageSizer.#clampToMinDimensions(width, height);
  }
   ...
   * @returns {{ width: number, height: number }}
   */
  static parseDimensions(dimensionStr) {
